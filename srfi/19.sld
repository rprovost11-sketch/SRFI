;; SRFI 19 -- Time Data Types and Procedures.  A pure-R7RS adaptation of the
;; canonical reference implementation (github.com/scheme-requests-for-implementation/
;; srfi-19, file srfi-19.scm), so the SAME file loads identically on pyScheme and
;; cppScheme2 (byte-identical by construction, no native part).
;;
;; Adaptations from the (Chicken-flavored) reference:
;;   * define-struct -> R7RS define-record-type.  time has mutable, exported setters;
;;     date has mutable fields with INTERNAL mutators (tm:set-date-*!) used by
;;     string->date / time-tai->date, but no public date setters (dates are immutable
;;     to users, as the SRFI requires).
;;   * receive -> let-values; (:optional x d) -> inline rest-arg test.
;;   * The wall-clock source is R7RS current-second / current-jiffy (from (scheme
;;     time)); the LOCAL time-zone offset comes from local-timezone-offset, the one
;;     non-R7RS host primitive, exported from (scheme time) so this library can import
;;     it.  current-time / current-date are therefore real (and DST-aware) but
;;     non-deterministic -- the test goldens exercise only deterministic procedures.
;;   * caddr/cadddr (in (scheme cxr)) -> list-ref; exact->inexact -> inexact.
;;
;; The leap-second table is current as of 2017 (TAI-UTC = 37 s).

(define-library (srfi 19)
  (export
   ;; constants
   time-duration time-monotonic time-process time-tai time-thread time-utc
   ;; current time and resolution
   current-date current-julian-day current-modified-julian-day current-time
   time-resolution
   ...
   time-monotonic->time-tai! time-monotonic->time-utc time-monotonic->time-utc!
   ...
   time-utc->time-monotonic time-utc->time-monotonic! time-utc->time-tai
   time-utc->time-tai!
   ;; string converters
  ...
  (begin
    ...
    (define tm:tai-epoch-in-jd 4881175/2)  ; julian day of the TAI epoch

    (define (tm:time-error caller type value)
      (error (string-append "srfi-19: " (symbol->string caller) ": "
                            (symbol->string type))
             value))

    (define (tm:floor-div a b) (floor (/ a b)))

    ;; ---- leap-second table (current through 2017: 37 s) ---------------------
    (define tm:leap-second-table
      '((1483228800 . 37) (1435708800 . 36) (1341100800 . 35) (1230768000 . 34)
        (1136073600 . 33) (915148800 . 32) (867715200 . 31) (820454400 . 30)
        (773020800 . 29) (741484800 . 28) (709948800 . 27) (662688000 . 26)
        (631152000 . 25) (567993600 . 24) (489024000 . 23) (425865600 . 22)
        (394329600 . 21) (362793600 . 20) (315532800 . 19) (283996800 . 18)
        (252460800 . 17) (220924800 . 16) (189302400 . 15) (157766400 . 14)
    ...
    ;; ---- current time and resolution ----------------------------------------
    ;; Wall clock from current-second (inexact epoch seconds); nanosecond field uses
    ;; the reference's 10000-ns granularity (resolution 10000), so current-time is
    ;; non-deterministic and not golden-tested -- it only needs to work.
    (define (tm:get-time-of-day)
      (let* ((now  (current-second))
             (secs (exact (floor now)))
             (ms   (exact (floor (* (- now (floor now)) 1000)))))
        (values secs ms)))
    ...
    (define (tm:current-time-tai)
      (let-values (((seconds ms) (tm:get-time-of-day)))
        (make-time time-tai (* ms 10000)
                   (+ seconds (tm:leap-second-delta seconds)))))

    (define (tm:current-time-monotonic)
      (let-values (((seconds ms) (tm:get-time-of-day)))
        (make-time time-monotonic (* ms 10000)
                   (+ seconds (tm:leap-second-delta seconds)))))

    (define (tm:current-time-ms-time time-type ms)
      (make-time time-type (* (remainder ms 1000) 10000) (quotient ms 1000)))

    (define (tm:current-time-process)
      (tm:current-time-ms-time time-process (current-jiffy)))
    (define (tm:current-time-thread)
      (tm:current-time-ms-time time-thread (current-jiffy)))
    ...
    (define (time=? t1 t2)
      (tm:time-compare-check t1 t2 'time=?)
    ...
    (define (tm:time-difference time1 time2 time3)
      ...
      (if (time=? time1 time2)
          (begin (set-time-second! time3 0) (set-time-nanosecond! time3 0))
    ...
    (define (tm:add-duration time1 duration time3)
      ...
      (if (not (eq? (time-type duration) time-duration))
          ...
          (let ((sec-plus  (+ (time-second time1) (time-second duration)))
            ...
            (let ((r (remainder nsec-plus tm:nano)) (q (quotient nsec-plus tm:nano)))
              (if (negative? r)
    ...
    (define (tm:subtract-duration time1 duration time3)
      (if (not (and (time? time1) (time? duration)))
          (tm:time-error 'subtract-duration 'incompatible-time-types #f))
      (if (not (eq? (time-type duration) time-duration))
          (tm:time-error 'subtract-duration 'not-duration duration)
          (let ((sec-minus  (- (time-second time1) (time-second duration)))
                (nsec-minus (- (time-nanosecond time1) (time-nanosecond duration))))
            (let ((r (remainder nsec-minus tm:nano)) (q (quotient nsec-minus tm:nano)))
              (if (negative? r)
    ...
    (define (time-monotonic->time-tai time-in)
      ...
      (let ((ntime (copy-time time-in))) (set-time-type! ntime time-tai) ntime))
    (define (time-monotonic->time-tai! time-in)
      (if (not (eq? (time-type time-in) time-monotonic))
          (tm:time-error 'time-monotonic->time-tai! 'incompatible-time-types time-in))
      (set-time-type! time-in time-tai) time-in)
    (define (time-utc->time-monotonic time-in)
      ...
      (let ((ntime (tm:time-utc->time-tai! time-in (make-time #f #f #f)
        ...
        (set-time-type! ntime time-monotonic) ntime))
    (define (time-utc->time-monotonic! time-in)
      (if (not (eq? (time-type time-in) time-utc))
          (tm:time-error 'time-utc->time-monotonic! 'incompatible-time-types time-in))
    ...
    (define (time-tai->time-monotonic time-in)
      ...
      (let ((ntime (copy-time time-in))) (set-time-type! ntime time-monotonic) ntime))
    (define (time-tai->time-monotonic! time-in)
      (if (not (eq? (time-type time-in) time-tai))
          (tm:time-error 'time-tai->time-monotonic! 'incompatible-time-types time-in))
      (set-time-type! time-in time-monotonic) time-in)

    ;; ---- the date record (immutable to users) -------------------------------
    ...
    (define (tm:encode-julian-day-number day month year)
      (let* ((a (quotient (- 14 month) 12))
        ...
        (+ day (quotient (+ (* 153 m) 2) 5) (* 365 y)
           (quotient y 4) (- (quotient y 100)) (quotient y 400) -32045)))
    ...
    (define (tm:local-tz-offset) (local-timezone-offset))
    ...
    (define (tm:find proc l)
      (cond ((null? l) #f) ((proc (car l)) #t) (else (tm:find proc (cdr l)))))
    ...
    (define (tm:time->date time tz-offset ttype)
      ...
      (let ((offset (if (pair? tz-offset) (car tz-offset) (tm:local-tz-offset))))
        (let-values (((secs date month year)
          ...
          (let* ((hours   (quotient secs (* 60 60)))
            ...
            (make-date (time-nanosecond time)
                       seconds minutes hours date month year offset)))))
    ...
    (define (date->time-utc date)
      (let ((nanosecond (date-nanosecond date)) (second (date-second date))
            (minute (date-minute date)) (hour (date-hour date))
            (day (date-day date)) (month (date-month date))
            (year (date-year date)) (offset (date-zone-offset date)))
        (let ((jdays (- (tm:encode-julian-day-number day month year)
                        tm:tai-epoch-in-jd)))
          (make-time time-utc nanosecond
                     (+ (* (- jdays 1/2) 24 60 60) (* hour 60 60)
    ...
    (define (tm:week-day day month year)
      (let* ((a (quotient (- 14 month) 12)) (y (- year a)) (m (+ month (* 12 a) -2)))
    ...
    (define (tm:date-week-number-iso date)
      (let* ((first-day-of-the-week (tm:week-day 1 1 (date-year date)))
             ...
             (w (+ (tm:floor-div (+ (date-year-day date) first-day-of-the-week -2) 7)
                   offset)))
        (cond ((zero? w)
               (tm:date-week-number-iso
                (make-date 0 0 0 0 31 12 (- (date-year date) 1) 0)))
              ((and (= w 53) (<= (tm:week-day 1 1 (+ (date-year date) 1)) 4)) 1)
              (else w))))
    ...
    (define (date->julian-day date)
      (let ((nanosecond (date-nanosecond date)) (second (date-second date))
            ...
            (day (date-day date)) (month (date-month date))
            (year (date-year date)) (offset (date-zone-offset date)))
        ;; NOTE: the SRFI 19 reference impl divides by (- offset) here, which is a
        ;; bug -- it raises division-by-zero on any UTC (offset 0) date.  The correct
        ;; form (matching date->time-utc) subtracts the offset from the day-seconds
        ;; then divides by seconds-in-a-day; verified J2000 -> 2451545.
        (+ (tm:encode-julian-day-number day month year) (- 1/2)
           (/ (- (+ (* hour 60 60) (* minute 60) second (/ nanosecond tm:nano))
                 offset)
              tm:sid))))
    ...
    (define (time-utc->julian-day time)
      ...
      (+ (/ (+ (time-second time) (/ (time-nanosecond time) tm:nano)) tm:sid)
         tm:tai-epoch-in-jd))
    ...
    (define (time-tai->julian-day time)
      ...
      (+ (/ (+ (- (time-second time) (tm:leap-second-delta (time-second time)))
               (/ (time-nanosecond time) tm:nano)) tm:sid)
         tm:tai-epoch-in-jd))
    ...
    (define (julian-day->time-utc jdn)
      (let ((nanosecs (* tm:nano tm:sid (- jdn tm:tai-epoch-in-jd))))
        (make-time time-utc (remainder nanosecs tm:nano)
    ...
    (define (current-date . tz-offset)
      (time-utc->date (current-time time-utc)
                      (if (pair? tz-offset) (car tz-offset) (tm:local-tz-offset))))
    ...
    (define (tm:locale-abbr-weekday n) (vector-ref tm:locale-abbr-weekday-vector n))
    (define (tm:locale-long-weekday n) (vector-ref tm:locale-long-weekday-vector n))
    (define (tm:locale-abbr-month n)   (vector-ref tm:locale-abbr-month-vector n))
    (define (tm:locale-long-month n)   (vector-ref tm:locale-long-month-vector n))
      ...
      (let ((len (vector-length haystack)))
        (let loop ((index 0))
          (cond ((>= index len) #f)
                ...
                (else (loop (+ index 1)))))))
    (define (tm:locale-abbr-weekday->index s)
      (tm:vector-find s tm:locale-abbr-weekday-vector string=?))
    (define (tm:locale-long-weekday->index s)
      (tm:vector-find s tm:locale-long-weekday-vector string=?))
    (define (tm:locale-abbr-month->index s)
      (tm:vector-find s tm:locale-abbr-month-vector string=?))
    (define (tm:locale-long-month->index s)
      (tm:vector-find s tm:locale-long-month-vector string=?))
      ...
      (let* ((str (number->string n)) (str-len (string-length str)))
        (if (or (> str-len length) (not pad-with))
            ...
            (let* ((new-str (make-string length pad-with))
              ...
              new-str))))
    (define (tm:last-n-digits i n) (abs (remainder i (expt 10 n))))
    (define (tm:locale-am/pm hr) (if (> hr 11) tm:locale-pm tm:locale-am))
    ...
    (define (tm:locale-print-time-zone date port) (if #f #f))
      ...
      (list
       ...
       (cons #\I (lambda (date pad-with port)
                   (let ((hr (date-hour date)))
                     (cond ((zero? hr) (display (tm:padding 12 pad-with 2) port))
       ...
       (cons #\l (lambda (date pad-with port)
                   (let ((hr (date-hour date)))
                     (cond ((zero? hr) (display (tm:padding 12 #\space 2) port))
                           ((> hr 12) (display (tm:padding (- hr 12) #\space 2) port))
                           (else (display (tm:padding hr #\space 2) port))))))
       ...
       (cons #\V (lambda (date pad-with port)
                   (display (tm:padding (tm:date-week-number-iso date) #\0 2) port)))
       ...
       (cons #\y (lambda (date pad-with port)
                   (display (tm:padding (tm:last-n-digits (date-year date) 2)
                                        pad-with 2) port)))
       (cons #\Y (lambda (date pad-with port)
                ...
                (if (= (+ index 1) str-len)
                    ...
                    (let ((pad-char? (string-ref format-string (+ index 1))))
                      (cond
                       ((char=? pad-char? #\-)
                        (if (= (+ index 2) str-len)
                            ...
                            (let ((formatter (tm:get-formatter
                                              (string-ref format-string (+ index 2)))))
                              (if (not formatter)
                                  (tm:time-error 'tm:date-printer 'bad-date-format-string format-string)
                       ...
                       (else
                        (let ((formatter (tm:get-formatter
                                          (string-ref format-string (+ index 1)))))
                          (if (not formatter)
                              (tm:time-error 'tm:date-printer 'bad-date-format-string format-string)
    ...
    (define (date->string date . format-string)
      (let ((str-port (open-output-string))
            (fmt-str (if (pair? format-string) (car format-string) "~c")))
        (tm:date-printer date 0 fmt-str (string-length fmt-str) str-port)
        (get-output-string str-port)))

    ;; ---- string->date -------------------------------------------------------
    (define (tm:char->int ch)
      (cond ((char=? ch #\0) 0) ((char=? ch #\1) 1) ((char=? ch #\2) 2)
            ((char=? ch #\3) 3) ((char=? ch #\4) 4) ((char=? ch #\5) 5)
            ((char=? ch #\6) 6) ((char=? ch #\7) 7) ((char=? ch #\8) 8)
            ((char=? ch #\9) 9)
            (else (tm:time-error 'string->date 'bad-date-template-string ch))))

    (define (tm:integer-reader upto port)
      (let loop ((accum 0) (nchars 0))
        (let ((ch (peek-char port)))
          (if (or (eof-object? ch) (not (char-numeric? ch))
                  (and upto (>= nchars upto)))
              accum
              (loop (+ (* accum 10) (tm:char->int (read-char port))) (+ nchars 1))))))
    (define (tm:make-integer-reader upto) (lambda (port) (tm:integer-reader upto port)))
          ...
          (let ((ch (peek-char port)))
            (cond ((>= nchars n) accum)
                  ...
                  ((char-numeric? ch)
                   (set! padding-ok #f)
                   (loop (+ (* accum 10) (tm:char->int (read-char port))) (+ nchars 1)))
                  (padding-ok (read-char port) (loop accum (+ nchars 1)))
    ...
    (define (tm:zone-reader port)
      (let ((offset 0) (pos? #f))
        (let ((ch (read-char port)))
          (if (eof-object? ch)
              (tm:time-error 'string->date 'bad-date-template-string ch))
          (if (or (char=? ch #\Z) (char=? ch #\z))
              0
              (begin
                (cond ((char=? ch #\+) (set! pos? #t))
                      ((char=? ch #\-) (set! pos? #f))
                      (else (tm:time-error 'string->date 'bad-date-template-string ch)))
                (let ((ch (read-char port)))
                  (if (eof-object? ch)
        ...
        (let loop ()
          (let ((ch (peek-char port)))
            (if (and (not (eof-object? ch)) (char-alphabetic? ch))
                (begin (write-char (read-char port) sp) (loop)))))
        (let* ((str (get-output-string sp)) (index (indexer str)))
          (if index index
              (tm:time-error 'string->date 'bad-date-template-string str)))))
    (define (tm:make-locale-reader indexer)
      (lambda (port) (tm:locale-reader port indexer)))
    (define (tm:make-char-id-reader char)
      (lambda (port)
        (if (char=? char (read-char port)) char
            (tm:time-error 'string->date 'bad-date-template-string char))))
    ...
    (define tm:read-directives
      (let ((ireader4 (tm:make-integer-reader 4))
            ...
            (do-nothing (lambda (val object) (if #f #f))))
        (list
         ...
         (list #\z (lambda (c) (or (char=? c #\Z) (char=? c #\z)
                                   (char=? c #\+) (char=? c #\-)))
               tm:zone-reader
                    ...
                    (let* ((format-char (string-ref format-string (+ index 1)))
                      ...
                      (if (not format-info)
                          ...
                          (let ((skipper (cadr format-info))
                                (reader (list-ref format-info 2))
                                (actor (list-ref format-info 3)))
    ...
    (define (string->date input-string template-string)
      (define (tm:date-ok? date)
        (and (date-nanosecond date) (date-second date) (date-minute date)
             (date-hour date) (date-day date) (date-month date)
             (date-year date) (date-zone-offset date)))
